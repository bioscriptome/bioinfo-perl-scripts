...
#-------------------------------------------------------------------------------
    ...
    return(&errSbr("miss in file '$fileInLoc'!"))  if (! -e $fileInLoc);
                                # ------------------------------
				# local defaults
    $exeFormatdbLocDef=            "/home/nishant/progs/VSL2/molbio/formatdb.$ARCH";
                                # ------------------------------
				# check arguments
...
#-------------------------------------------------------------------------------
    ...
    return(&errSbr("miss in file '$fileInLoc'!"))  if (! -e $fileInLoc);
                                # ------------------------------
				# local defaults
    $exeBlast3LocDef=           "/home/nishant/progs/VSL2/molbio/blastall.$ARCH";
    $dbBlastLocDef=             "swiss";
    $parELocDef=             1000;
    $parBLocDef=             2000;
                                # ------------------------------
...
#-------------------------------------------------------------------------------
    ...
    return(&errSbr("miss in file '$fileInLoc'!"))  if (! -e $fileInLoc);
                                # ------------------------------
				# local defaults
    $exeBlastpLocDef=           "/home/nishant/progs/VSL2/molbio/blastp.$ARCH";
    $dbBlastLocDef=             "swiss";
    $parELocDef=             1000;
    $parBLocDef=             2000;
...
#-------------------------------------------------------------------------------
    ...
    return(&errSbr("miss in file '$fileInLoc'!"))  if (! -e $fileInLoc);
                                # ------------------------------
				# local defaults
    $exeFormatdbLocDef=         "/home/nishant/progs/VSL2/molbio/setdb.$ARCH";
                                # ------------------------------
				# check arguments
    $titleLoc=0                 if (! defined $titleLoc);
    $exeFormatdbLoc=$exeFormatdbLocDef 
...
#-------------------------------------------------------------------------------
    ...
    $BLASTDBdef= "/home/nishant/progs/VSL2/molbio/db";
				# check arguments
...
#-------------------------------------------------------------------------------
    ...
    $exeConvSeq=    "/home/nishant/progs/VSL2/phd/bin/convert_seq.".$ARCH 
	                              if (! defined $exeConvSeq     || $exeConvSeq     eq "def");
    $exeBlastp=     "/home/phd/bin/".  $ARCH."/blastp"
	                              if (! defined $exeBlastp      || $exeBlastp      eq "def");
    $exeBlastpFil=  "/home/nishant/progs/VSL2/max/scr/filter_blastp" 
                                      if (! defined $exeBlastpFil   || $exeBlastpFil   eq "def");
    $exeMax=        "/home/nishant/progs/VSL2/max/bin/maxhom.".$ARCH
    ...
    $fileMaxDef=    "/home/nishant/progs/VSL2/max/maxhom.default" 
	                              if (! defined $fileMaxDef     || $fileMaxDef     eq "def");
    $fileMaxMetr=   "/home/nishant/progs/VSL2/max/mat/Maxhom_GCG.metric" 
...
#-------------------------------------------------------------------------------
    ...
    $envBlastDb= "/home/nishant/progs/VSL2/ncbi/db/"                if (! defined $envBlastDb);
    $envBlastMat="/home/pub/molbio/blast/blastapp/matrix" if (! defined $envBlastMat);
    $parBlastDb= "/home/nishant/progs/VSL2/ncbi/db/prodom"          if (! defined $parBlastDb);
...
#-------------------------------------------------------------------------------
    $sbrName="lib-br:"."segRun";$fhinLoc="FHIN_"."segRun";$fhoutLoc="FHIN_"."segRun";
				...
				# ------------------------------
				# defaults
    $exeSegDef=          "/home/nishant/progs/VSL2/molbio/bin/seg".".".$ARCH;
    $modeSegDef=         "glob";
    $winSegGlobDef=    30;
    $locutGlobDef=      3.5;
    $hicutGlobDef=      3.75;
