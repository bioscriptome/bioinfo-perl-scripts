...
# @d=1-5
    ...
    for ($d1=0;$d1<@d;$d1++)
    {
	    ...
	    system "~/progs/svm_learn -z c -t 1 -d $d[$d1] cat_test model1_d";
	    system "~/progs/svm_classify eval_1 model1_d res1_d";
	    system "cat res1_d >>p_res1_d";
	
   
        print OH "\n\nKernel Parameters ........t=1 (Polynomial) d=$d[$d1] \n";
        system  "~/progs/create_table.pl o_res p_res1_d";
    }
#    system "rm res1* o_res cat_test  model1_d*";
    close OH;
    exit;
}
