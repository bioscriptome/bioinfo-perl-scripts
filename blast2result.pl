...
$tag=$opt_t;
$db1=$opt_y;
if($pos eq '' ||$db eq '' ||$db1 eq ''||$neg eq ''){print "-p: positive fasta file; 
...
if($dbt eq 'p' ){$db1="AVP_543";}
if($dbt eq 'n'){$db1="AVP_N_418";}
...
system " ~/blast-2.2.24/bin/blastall -p blastp -d $db/$db1 -i $pos -F F -f 9 -b $neg -e 100 -m 8 |cut -f1,2,11|sort -u >$fold/$tag\_pos";
