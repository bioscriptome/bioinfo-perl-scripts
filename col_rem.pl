#############################################################################
##    'col_rem' is free software: you can redistribute it and/or modify      #
...
# program developed for removing selective columns
...
$count = 0;
	...
	$c2 = $_;
	$count++;
	if(($count >= $cnum1) && ($count <= $cnum2)){$k1 = 0;}else{
...
}elsif($file1 ne '' && $cnum1 ne '' && $cnum2 ne ''){
...
close FP1;
    ...
    print "Usage: col_rem -i col_file -o col_output -a from_col_no -b tocol_no\n";
    print "Usage for displaying on screen: col_rem -i col_file -a from_col_no -b tocol_no\n";
}
