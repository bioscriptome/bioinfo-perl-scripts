...
if (($opt_i ne '')&&($opt_o ne ''))
	...
	if($c1 =~ ">")
	{
	    ...
	    $len=$le-$position;
	    for($i1=1; $i1 <= 20; $i1++){
	        for($i2=1; $i2 <= 20; $i2++){$comp[$i1][$i2]=0;}
	    }
	    print "\n";
	    for($j1 = 0; $j1 < $#ti1-$position+1; $j1++){
		$c1 = $ti1[$j1];
		$in1 = index($aa,$c1);
		$c2 = $ti1[$j1+$position];
		$in2 = index($aa,$c2);
		$comp[$in1][$in2]++;
		print "res1:".$c1." pos1:$in1  res2:$c2  pos2:$in2  comp $in1 $in2:$comp[$in1][$in2]\n";
	    }
	    $count=0;
	    for($i1=1; $i1 <= 20; $i1++){
	        for($i2=1; $i2 <= 20; $i2++)
		    {
		    $perc=(($comp[$i1][$i2])*10)/$len;
		    $count++;
		    if($count <= 399){printf(FP2 "%5.3f,",$perc);}
		    else{printf(FP2 "%5.3f",$perc);} 
		}
	    }
	    print FP2 "\n";
	}
