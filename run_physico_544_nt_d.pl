...
#open(FH2,$ARGV[1]);@negat=<FH2>;
    open(FH3,$ARGV[1]);#file having the PhysicoChemical_aaindex1 ids 
#eg ANDN920101
#ARGP820101
#ARGP820102
#ARGP820103
...
#    system "perl ~/progs/calcPhysicoChemical.pl ~/Matrix/physico/physicochemical_544/$id $ARGV[1] >negative.$id ";
  system "perl ~/progs/gpsrn/col2svm -i positve.$id -o positve.$id.svm1 -s +1";
#system "perl ~/progs/sipred_progs/mkactual_data_test.pl iscore
  system "paste -d ' ' ../dieter_2431_21_aa.sfasta.SCORE positve.$id.svm1 > positve.$id.svm";
system "perl ~/progs/sipred_progs/mk/run_progs_mks_d.pl -i 	positve.$id.svm -f 10 -t f -r w";

#system "mkdir ../id ../svm_result";
    system "mkdir ../id/$id";
    system "cp  averageRESULT ../svm_result/svm_result_$id";
    system "mv * ../id/$id/.";
    @array_svm=();
