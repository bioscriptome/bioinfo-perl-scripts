...
foreach $matrix(@mtx){
chomp($matrix);
@mtx_split=split("\t",$matrix);
$hash{$mtx_split[0]}=$mtx_split[1];
...
for($in=0;$in<scalar(@seq1);$in++){
	if($seq1[$in] eq $seq2[$in]){}
		...
		$end=($snp+18);
		print $start."\t".$end."\n";
		print @seq1[$start..$end];print "\n";
		print @seq2[$start..$end];print "\n";
		@a1=@seq1[$start..$end];
		$allele1=join("",@a1);
		#$revseq1=~tr/uU/tt/;
		...
		if($seq2[$in] eq 't'){$col=1;}
		if($seq2[$in] eq 'c'){$col=2;}
		if($seq2[$in] eq 'a'){$col=3;}
		if($seq2[$in] eq 'g'){$col=4;}
		$nt=$seq2[$in];
		$nt=~tr/tcga/agct/;
